...
function onPlayerReady(event) {
  ...
  if (pendingPlaylistLoad) {
    ...
    loadPlaylistById(pending.playlistId, pending.originalUrl, pending.startIndex, pending.startSeconds);
  } else {
    loadLastPlaylist();
  }
}
...
async function loadPlaylistById(playlistId, originalUrl = '', startIndex = 0, startSeconds = 0) {
  if (!ytPlayer || !ytPlayerReady) {
    pendingPlaylistLoad = { playlistId, originalUrl, startIndex, startSeconds };
  ...
  try {
    // Cue playlist into YouTube player
  ...
  try {
    ...
    if (activeId) {
      ...
      if (state && state.playlistId === activeId) {
        loadPlaylistById(activeId, activePL ? activePL.url : '', state.index, state.currentTime);
      } else {
        loadPlaylistById(activeId, activePL ? activePL.url : '');
      }
    }
  } catch (err) {
  ...
  if (playlistVideos.length > 0) {
    ...
    try {
      ...
      if (data.title) {
        ...
        // Save the updated title in cache
